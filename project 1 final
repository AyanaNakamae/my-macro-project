...
# 例としてアメリカ合衆国 (USA) を選択します。FREDのシリーズIDを使用します。
# 実質GDPの四半期データ (GDPC1) を取得します。
# 'YOUR_FRED_API_KEY' を実際のAPIキーに置き換えてください
...
for lam in lambdas:
  # hpfilterはタプルでトレンドと循環成分を返します
  trend, cycle = hpfilter(log_gdp, lam)
  trends[lam] = trend
  cycles[lam] = cycle
...
#Canada and Japan's Comparison Graph (Step 4.5)
